...
> Version applicative : **1.3** (Python / PyQt6) — versionnage repris à 1.0 lors du passage de P.R.I.S.M à LGS InstalleX ; la lignée technique précédente s'arrêtait à 3.8.1
...
- **Mode Personnalisation** — le catalogue `TACHES` décrit les 19 tâches sélectionnables et leur hiérarchie ; `machine_info["taches"]` vaut `None` en mode normal (tout s'exécute) ou porte l'ensemble des clés retenues. Un seul point de décision, `_actif(clé)`, est consulté par la boucle d'étapes et par chacun des blocs applicatifs de l'étape 6 — la sélection ne peut donc pas diverger d'un endroit à l'autre. Le mode ne **retire** jamais rien : il se contente de sauter, ce qui le rend sûr à relancer sur un poste déjà provisionné.
...
- **v1.3 (courant)** *(2026-08-25)* :
  - **Mode Personnalisation** — une case de l'écran de configuration ouvre la liste des **19 tâches** (les 9 étapes, plus les 10 logiciels de l'étape 6 pris séparément) et permet de n'exécuter que celles cochées. Répond à deux besoins concrets : ne réinstaller qu'un seul logiciel, ou reprendre une étape qui a échoué, sans repasser sur le reste. Rien n'est désinstallé — les tâches non retenues sont simplement sautées, et celles qui s'exécutent commencent toutes par vérifier ce qui est déjà en place : relancer une tâche déjà faite reste sans effet.
